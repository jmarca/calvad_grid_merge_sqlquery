...
describe('post process sql queries 2012',function(){
    it('should  handle a detectorized grid cell'
      ,function(done){
           ...
           .await(function(err,atask,btask){
               ...
               // on just this test
               task.should.have.property('accum').with.lengthOf(6);
               ...
               reduce.post_process_sql_queries(task,function(err,cbtask){
                   ...
                   _.size(task.aadt_store).should.eql(5)
                   task.aadt_store.should.have.property('CO P')
                   task.aadt_store['CO P']
                   .should.have.keys('sum_vmt'
                                    ,'sum_lane_miles'
                                    ,'sum_single_unit_mt'
                                    ,'sum_combination_mt')
                   task.aadt_store['CO P'].should.have.property( 'sum_vmt').with.approximately(           (70747),0.1)
                   task.aadt_store['CO P'].should.have.property( 'sum_lane_miles').with.approximately(    (0  ),0.01)
                   task.aadt_store['CO P'].should.have.property( 'sum_single_unit_mt').with.approximately((0      ),0.01)
                   task.aadt_store['CO P'].should.have.property( 'sum_combination_mt').with.approximately((0         ),0.01)

                   // 101 exists in detector set for this grid cell
                   task.aadt_store.should.not.have.property('SHS_101')
                   task.aadt_store.should.have.property('SHS_192')
                   task.aadt_store['SHS_192']
                   .should.have.keys('sum_vmt'
                                    ,'sum_lane_miles'
                                    ,'sum_single_unit_mt'
                                    ,'sum_combination_mt')
                   task.aadt_store['SHS_192'].should.have.property( 'sum_vmt').with.approximately(           (17199),0.1)
                   task.aadt_store['SHS_192'].should.have.property( 'sum_lane_miles').with.approximately(    (0 ),0.01)
                   task.aadt_store['SHS_192'].should.have.property( 'sum_single_unit_mt').with.approximately((0 ),0.01)
                   task.aadt_store['SHS_192'].should.have.property( 'sum_combination_mt').with.approximately((0 ),0.01)

                   task.aadt_store.should.have.property('SHS_225')
                   task.aadt_store['SHS_225']
                   .should.have.keys('sum_vmt'
                                    ,'sum_lane_miles'
                                    ,'sum_single_unit_mt'
                                    ,'sum_combination_mt')
                   task.aadt_store['SHS_225'].should.have.property( 'sum_vmt').with.approximately(2301,0.1)
                   task.aadt_store['SHS_225'].should.have.property( 'sum_lane_miles').with.approximately(0,0.01)
                   task.aadt_store['SHS_225'].should.have.property( 'sum_single_unit_mt').with.approximately(0,0.01)
                   task.aadt_store['SHS_225'].should.have.property( 'sum_combination_mt').with.approximately(0,0.01)
                   ...
                   reduce.reduce_aadt_store(task,function(err,t3){
                       ...
                       task.aadt_totals.should.have.property( 'sum_vmt').with.approximately           (70747+3049+151786+17199+2301,0.1)
                       task.aadt_totals.should.have.property( 'sum_lane_miles').with.approximately    (0,0.1)
                       task.aadt_totals.should.have.property( 'sum_single_unit_mt').with.approximately(0,0.1)
                       task.aadt_totals.should.have.property( 'sum_combination_mt').with.approximately(0,0.1)
...
describe('post process sql queries 2014',function(){
    it('should  handle a detectorized grid cell'
      ,function(done){
           var task={'cell_id':'189_72'
                    ,'year':2014
