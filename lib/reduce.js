    ...
    _.each(task.aadt_store,function(aadt_record,road_class){
        ...
        _.each(task.fractions,function(record,timestamp){
            ...
            task.accum[timestamp][road_class]['sum_lane_miles']
             = record.nhh * aadt_record['sum_lane_miles']
        });

        _.each(nhh_vars,function(k2){
            // multiply by nhh
            _.each(task.fractions,function(record,timestamp){
                task.accum[timestamp][road_class][k2] = record.nhh * aadt_record[k2]
            });
        });
        _.each(n_vars,function(k2){
            // multiply by n
            _.each(task.fractions,function(record,timestamp){
                task.accum[timestamp][road_class][k2] = record.n * aadt_record[k2]
            });
        });
        _.each(hh_vars,function(k2){
            // multiply by n
            _.each(task.fractions,function(record,timestamp){
                task.accum[timestamp][road_class][k2] = record.hh * aadt_record[k2]
            });
        });
    });
