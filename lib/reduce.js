...
function reduce(memo,item,callback){
    ...
    // either item.accum or item.detector_data
    console.log({"item.accum":_.size(item.accum)
                ,"item.detector_data":_.size(item.detector_data)})
    ...
    _.each(item.accum,function(roads,ts){
        if(memo[ts]===undefined){
            ...
            _.each(roads,function(record,road_class){
                ...
                var detector_miles = item.detector_data[unmapper.miles]
                ...
                if(memo[ts]['detector_based']===undefined){
                    memo[ts]['detector_based']={'n':item.detector_data[unmapper.n]
                                               ,'n_mt':item.detector_data[unmapper.n]*detector_miles
                                               ,'hh_mt':item.detector_data[unmapper.hh]*detector_miles
                                               ,'nhh_mt':item.detector_data[unmapper.not_hh]*detector_miles
                                               ,'lane_miles':item.detector_data[unmapper.lane_miles]
                                               }
                }else{
                    console.log(item)
                    console.log(unmapper)
                    console.log({"item.detector_data[unmapper.n]":item.detector_data[unmapper.n]})

                    memo[ts]['detector_based'].n      += item.detector_data[unmapper.n]
                    memo[ts]['detector_based'].n_mt   += item.detector_data[unmapper.n]*detector_miles
                    memo[ts]['detector_based'].hh_mt  += item.detector_data[unmapper.hh]*detector_miles
                    memo[ts]['detector_based'].nhh_mt += item.detector_data[unmapper.not_hh]*detector_miles
                    memo[ts]['detector_based'].lane_miles += item.detector_data[unmapper.lane_miles]
                }
            })
        }
    });
