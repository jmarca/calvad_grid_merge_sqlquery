 ...
 * @returns {} null
 */
var get_hpms_from_sql=function(task,pool,cb){
    // if task has detector data, then have to be careful to ID routes
    // that match
    // sql

    // if year > 2009, switch to that one
    if(task.year > 2009) return other.get_hpms_from_sql_2014(task,pool,cb)
...
var get_detector_route_nums = function(task,pool,cb){
