...
var get_hpms_from_sql=function(task,cb){
    ...
    if(task.fips === undefined ){
        task.fips = cellmembership[task.cell_id].fips
    }
    ...
    var  county_with_statement =
        ...
        "    ,CASE WHEN is_metric>0 THEN section_length*clipped_fraction*0.621371 ELSE section_length*clipped_fraction END as sec_len_miles",
    ...
    var grids_with_statement =
        ["hpmsgrids_all as",
         "( select  hc.*, floor(grids.i_cell) || '_'|| floor(grids.j_cell) as cell",
         "     ,st_length( (ST_Dump(ST_Intersection(grids.geom4326, hg.geom))).geom) as clipped_length, hd.direction",
         "  from hpms_county hc",
         "  join carbgrid.state4k grids on(floor(grids.i_cell) || '_'|| floor(grids.j_cell)='"+task.cell_id+"')",
         ...
         ")",
         ", hpmsgrids_summed as (",
         "  select cell, hpms_id, sum(clipped_length) as clipped_length from hpmsgrids_all group by cell, hpms_id",
         ")",
         ", hpms_links as (",
         ...
         ")",
         ", hpms_joined as (",
         "  select hga.*, clipped_fraction",
         "  from hpmsgrids_al hga",
