 ...
 * @returns {} null
 */
var get_hpms_from_sql_2014=function(task,pool,cb){
    // if task has detector data, then have to be careful to ID routes
    // that match
    // sql
    var grid_with_statement,county_with_statement,joined_with_statement,select_statement,query
    var year = 2014
    ...
    var pass = task.options.postgresql.password
    ...
    if (task.year !== undefined && task.year > 2014){
        year = task.year
    }
    ...
    county_with_statement =[
        ...
        "  from grid_cell",
        "  join hpms.hpms_count_data hd  on ( st_intersects(geom4326,st_transform(geom,4326)) )",
    ...
    select_statement =[
        ...
        //,"       -- hpms_id,route_id,",
        ,"       sum(aadt) as sum_aadt,",
        ,"       sum(aadt_combination) as sum_aadt_combination,",
        ,"       sum(aadt_single_unit) as sum_aadt_single_unit,",
        ,"       floor(sum(aadt*sec_len_miles)) as sum_vmt,",
        ,"       floor(sum(aadt_combination*sec_len_miles)) as sum_combination_mt,",
        ,"       floor(sum(aadt_single_unit*sec_len_miles)) as sum_single_unit_mt",
        //,"--       ,sum(sec_len_miles*through_lanes) as sum_lane_miles",
        //,"--       ,floor(sum(avg_single_unit*aadt/100)) as sum_single_unit",
        //,"--       ,floor(sum(avg_single_unit*aadt*sec_len_miles/100)) as sum_single_unit_mt",
        //,"--       ,floor(sum(avg_combination*aadt/100)) as sum_combination",
    ...
    query = 'with '
              ...
              + select_statement
    //console.log(query)
    // switch to using pool, not pg.connect
